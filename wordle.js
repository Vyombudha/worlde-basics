...
function handleInput(e){
    ...
    let tiles = Array.from(rows[currentRowIndex].querySelectorAll("p"));

    if(e.key === 'Backspace'){
        if(currentTileIndex > 0){
            currentTileIndex--;
            tiles[currentTileIndex].innerText = '';
        }
    }
    else if(e.key == 'Enter'){
        ...
        if(currentTileIndex < 5){
            // incomplete word — shake
            rows[currentRowIndex].classList.add('shake');
            setTimeout(() => rows[currentRowIndex].classList.remove('shake'), 400);
            return;
        }

        let userGuess = tiles.map((tiles) => tiles.innerText);
        const {winner , nextTile}=  checkAnswer(userGuess);
        ...
        if(currentTileIndex < 5){
            tiles[currentTileIndex].innerText  = e.key.toUpperCase();
            currentTileIndex++;
        }
    }
...
letters.forEach((letter) => {
    letter.addEventListener('click', (e) => {
        if(letter.innerText === "ENTER"){
            key = 'Enter';
        }
        else if(letter.dataset.key === 'Backspace'){
            key = 'Backspace';
        }
        else{
            key =  letter.innerText.toLowerCase();
        }
        handleInput({key});
    })
})
...
let correctGuess = getValidAnswer();
let gameOver = false;



let selectionBtn = document.querySelector('.difficulty-selection');

selectionBtn.addEventListener("change" , (e) => {
  let new_difficulty = parseInt(e.target.value);
  changeDifficulty(new_difficulty);
...
function checkAnswer(userGuess){
    ...
    if(!VALID_WORDS.has(userGuess.join(""))){
        ...
        setTimeout(() => {
            rows[currentRowIndex].classList.remove('shake')
        }, 400);
        ...
        return {nextTile : false, winner : false};
    }
    

    // check 1 (check for exact matches) , sets exact matches to null in correctGuess string after flagging

    for(let i = 0 ; i < 5 ; i++){
        if(userGuess[i] === answerArr[i]){
            matchesArr[i] = 'correct';
            answerArr[i] = null;
        }
    }

    // check 2  (check for wrongIndex and invalid matches) , sets wrongIndices to null after flagging

   for(let i = 0 ; i < 5 ; i++){
        if(matchesArr[i]) continue; // already flagged in check 1 

        const foundAt = answerArr.indexOf(userGuess[i]);

        if (foundAt !== -1) {
            matchesArr[i] = 'exists';
            answerArr[foundAt] = null; // out where it was found to null .. soo next time we don't check it in list
        } 
        else {
            matchesArr[i] = 'wrong';
        }
    ...
    tiles.forEach((tile,i) => {
        ...
        setTimeout(() => {
            ...
            tile.classList.add(matchesArr[i]);


        }, i * 300)  // each tile flips 300ms after the previous
    ...
    for(let i = 0 ; i < 5; i++){
        if(userGuess[i] !== correctGuess[i]){
            return {nextTile : true , winner : false};
        }
    }
    return {nextTile : false , winner : true};
